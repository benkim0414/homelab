#!/usr/bin/env bash
set -euo pipefail
INPUT=$(cat)  # consume required hook JSON input

# Node summary (name, role-derived status, Ready condition)
...
# Unhealthy pod count (not Running and not Succeeded)
UNHEALTHY=$(kubectl get pods -A --no-headers \
  --field-selector 'status.phase!=Running,status.phase!=Succeeded' \
  2>/dev/null | grep -c . || echo "?")

# ArgoCD app status — count OutOfSync or Degraded
